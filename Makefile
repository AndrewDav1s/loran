...
mocks:
	...
	@go run github.com/golang/mock/mockgen -destination=mocks/cosmos.go \
			-package=mocks github.com/cicizeo/loran/cmd/loran/client \
			CosmosClient
	@go run github.com/golang/mock/mockgen -destination=mocks/evm_provider.go \
			-package=mocks github.com/cicizeo/loran/orchestrator/ethereum/provider \
			EVMProviderWithRet
	@go run github.com/golang/mock/mockgen -destination=mocks/peggy_queryclient.go \
			-package=mocks github.com/cicizeo/hilo/x/peggy/types \
			QueryClient
	@go run github.com/golang/mock/mockgen -destination=mocks/peggy/peggy_contract.go \
			-package=peggy github.com/cicizeo/loran/orchestrator/ethereum/peggy \
			Contract
...
docker-build:
	@docker build -t hilonet/loran .

docker-build-debug:
	@docker build -t hilonet/loran --build-arg IMG_TAG=debug .
