...
func TestLoran(t *testing.T) {
	...
	BeforeSuite(func() {
		CoverageEnabled = toBool(os.Getenv("LORAN_TEST_COVERAGE")) || toBool(os.Getenv("COVERAGE"))

		if CoverageEnabled {
			CoverageAgent = deployer.NewCoverageDataCollector(
				deployer.CoverageMode(os.Getenv("LORAN_TEST_COVERAGE_MODE")),
			)
		}
	...
	AfterSuite(func() {
		if CoverageEnabled {
			...
			coverageOut := os.Getenv("LORAN_TEST_COVERAGE_OUT")
