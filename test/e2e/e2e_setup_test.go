...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	var useGanache bool
	if str := os.Getenv("LORAN_E2E_USE_GANACHE"); len(str) > 0 {
	...
	//
	// 1. Initialize Hilo validator nodes.
	// 2. Launch an Ethereum (or Ganache) container that mines.
	// 3. Create and initialize Hilo validator genesis files (setting delegate keys for validators).
	// 4. Start Hilo network.
	// 5. Deploy the Gravity Bridge contract
	// 6. Create and start loran (orchestrator) containers.
	s.initNodes()
	if useGanache {
...
func (s *IntegrationTestSuite) TearDownSuite() {
	if str := os.Getenv("LORAN_E2E_SKIP_CLEANUP"); len(str) > 0 {
...
func (s *IntegrationTestSuite) registerValidatorOrchAddresses() {
	s.T().Log("registering Hilo validator Ethereum keys...")
