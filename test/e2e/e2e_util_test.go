...
import (
	...
	peggycli "github.com/cicizeo/hilo/x/peggy/client/cli"
...
func (s *IntegrationTestSuite) deployERC20Token(baseDenom string) string {
	...
	s.Require().Eventually(
		func() bool {
			addr, cosmosNative, err := queryDenomToERC20(hiloAPIEndpoint, baseDenom)
			if err != nil {
				return false
			}
...
func (s *IntegrationTestSuite) registerOrchAddresses(valIdx int, hiloFee string) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
...
func (s *IntegrationTestSuite) sendFromHiloToEth(valIdx int, ethDest, amount, hiloFee, gravityFee string) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()

	s.T().Logf(
		"sending tokens from Hilo to Ethereum; from: %s, to: %s, amount: %s, hiloFee: %s, gravityFee: %s",
		s.chain.validators[valIdx].keyInfo.GetAddress(), ethDest, amount, hiloFee, gravityFee,
	)
