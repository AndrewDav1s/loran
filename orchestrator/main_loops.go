...
const (
	...
	//
	// Ref: https://github.com/cicizeo/loran/issues/55
...
func (p *gravityOrchestrator) EthOracleMainLoop(ctx context.Context) (err error) {
	...
	if err := retry.Do(func() (err error) {
		...
		if err != nil || gravityParamsResp == nil {
			logger.Fatal().Err(err).Msg("failed to query Gravity params, is hilod running?")
		}
	...
	return loops.RunLoop(ctx, p.logger, p.ethereumBlockTime*ethOracleLoopMultiplier, func() error {
		...
		//	1. It takes some time for events to be indexed on Ethereum. So if loran queried events immediately as
		...
		//	3. If the ETH call failed while filtering events, the loran missed to broadcast claim events occurred in
		//	   last iteration.
