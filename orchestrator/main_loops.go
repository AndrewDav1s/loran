	...
	return loops.RunLoop(ctx, defaultLoopDur, func() error {
		...
		/*
			Auto re-sync to catch up the nonce. Reasons why event nonce fall behind.
				1. It takes some time for events to be indexed on Ethereum. So if loran queried events immediately as block produced, there is a chance the event is missed.
				...
				3. if infura call failed while filtering events, the loran missed to broadcast claim events occured in last iteration.
		**/
